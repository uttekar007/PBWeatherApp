...
class SearchCityViewController: UIViewController, CLLocationManagerDelegate, UITableViewDelegate, UITableViewDataSource, UIGestureRecognizerDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        geocoder.reverseGeocodeLocation(location, completionHandler: {(placemarks, error) in
            ...
            if  error == nil && pm.count > 0 {
               ...
               // print("Location: \(self.currentPlace?.woeid!)")
                self.cityName.text = self.currentPlace?.name!
                // self.loadWeatherData(city: currentPlace.woeid!)
                self.loadCountryData(city: (self.currentPlace?.woeid!)!)
